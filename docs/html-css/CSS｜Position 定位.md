...
::::warning
**常見陷阱**：忘記設父層 `position: relative`，導致元素跑到非預期位置。
::::
...
::::warning
**常見陷阱**：父層有 `transform` 或 `filter` 時，`fixed` 會改以該父層為基準（行為異常）。
::::
...
::::warning
**常見陷阱**：父層設了 `overflow: hidden / auto / scroll`，sticky 會**失效**。
::::

### 三、偏移與堆疊

#### 1. `top / right / bottom / left` 偏移規則
