...
tags: [CSS, HTML, 課程筆記]
date: 2025-08-06
slug: /docs/html-css/web-animations
---
...
- **常使用的屬性：**
  ...
  - `transition-timing-function`：套用的[時間方法](https://developer.mozilla.org/en-US/docs/Web/CSS/animation-timing-function)
    → 上面三者可以合併撰寫
- **套用技巧：**
  - 原始 Class 套用 `transition` 屬性
  - 偽類（[虛擬類別](https://developer.mozilla.org/zh-TW/docs/Web/CSS/Pseudo-classes)）加入變動後的效果
  - 時間建議落在 `.25s` ~ `.5s` 之間即可
  - 範例：色彩變幻、旋轉
...
- Animation 延伸閱讀：
  ...
  - 本週可以做到的範例：[https://goldalles.com/](https://goldalles.com/)
...
- 老師，我還想要…
  - 參數：設定值，用來定義 Swiper 的運作
  - Methods、Event：進階細微操作 Swiper 的方法，例如當「用戶進行特定操作後輪播至下一頁」、「當輪播下一頁時，觸發特定方法」
