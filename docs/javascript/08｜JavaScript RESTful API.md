...
- **API 文件**：資料交換的使用說明書，提供規範的文字說明版本。
    ...
    ![API文件基本格式1](/img/js08-2.png)

    ![API文件基本格式2](/img/js08-3.png)
...
- **2021 前端/UI 問卷調查 API**：
  ...
  ![問卷調查API文件範例](/img/js08-4.png)

- **TDX 運輸資料流通平台 API**：
  - **Endpoint (端點)**：
    - [點此連結](https://tdx.transportdata.tw/api/basic/v2/Tourism/ScenicSpot?%24top=30&%24format=JSON)
  ...
  - **資料格式**：JSON
    ...
    ![TDX景點API文件範例](/img/js08-5.png)

- **NEWS 新聞 API**：
  - **Endpoint (端點)**：
    - [點此連結](https://newsapi.org/v2/everything?q=tesla&from=2025-09-30&sortBy=publishedAt&apiKey=API_KEY)
  - **網址參數**：
    ```
  ...
  - **資料格式**：JSON
      ...
      ![NEWS_API文件範例](/img/js08-6.png)

- **Google Maps API**：功能強大且資訊多，文件結構較複雜，需詳細閱讀其開發文件。
  
      ![GoogleMap_API文件範例](/img/js08-7.png)

- **最終專案任務 API**：使用 Swagger 服務建置的 API 文件。
  
       ![最終任務API文件範例](/img/js08-8.png)

#### 8. API 串接權限的四種分類

- 依安全性與限流分為：
  - **無限制、免驗證**：可在瀏覽器端 (JS) 直接讀取使用，不需申請帳號（如：政府的開放資料 Open Data）。
  - **需金鑰、非敏感**：可在瀏覽器端 (JS) 讀取使用，但需申請帳號取得金鑰，常有限流控制（如：Google Maps API）。
  - **限後端、免驗證**：因流量考量或跨網域限制，限伺服器後端讀取，不需帳號。
  - **限後端、需驗證**：安全性最高。因涉及敏感資料（如個資、金流），必須限制在後端讀取，且需帶金鑰驗證（如：News API）。
